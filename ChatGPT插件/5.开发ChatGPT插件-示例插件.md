...
为了开始构建，我们提供了一组涵盖不同身份验证模式和用例的简单插件。从我们简单的无需身份验证的待办事项列表插件到更[强大的检索插件](https://github.com/openai/chatgpt-retrieval-plugin/)，这些示例让我们得以一窥我们希望通过插件实现的目标。
...
我们创建了插件快速入门，作为开发人员在不到 5 分钟内启动和运行插件的起点。如果您还没有运行过插件并且想熟悉运行一个插件所需的最少步骤，请考虑从插件[quickstart repo](https://github.com/openai/plugins-quickstart)开始。
...
最后，我们需要设置和定义 OpenAPI 规范以匹配本地或远程服务器上定义的端点。您无需通过规范公开 API 的全部功能，而是可以选择让 ChatGPT 仅访问某些功能。
...
接下来，我们可以定义 API 端点来为特定用户创建、删除和获取待办事项列表项。端点还检查用户是否已通过身份验证。
...
最后，我们需要设置和定义 OpenAPI 规范以匹配远程服务器上定义的端点。通常，无论身份验证方法如何，OpenAPI 规范看起来都是一样的。使用自动 OpenAPI 生成器将减少创建 OpenAPI 规范时出错的机会。
...
要创建 OAuth 插件，我们首先定义一个`ai-plugin.json`auth 类型设置为的文件`oauth`：
...
接下来，我们需要定义我们的 OAuth 服务。此 OAuth 示例不适用于生产用例，而是强调简单的 OAuth 流程是什么样子的，以便开发人员可以获得构建生产解决方案的经验。
...
最后，与我们的其他示例一样，我们基于端点定义了一个简单的 OpenAPI 文件：
...
ChatGPT[检索插件](https://github.com/openai/chatgpt-retrieval-plugin)是一个功能更全面的代码示例。该插件的范围很大，因此我们鼓励您通读代码，看看更高级的插件是什么样子的。
